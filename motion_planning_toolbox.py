...
def check_path(point_current, point_other, map_img):
    ...
    #print(type(step_length))

    path_x = np.linspace(point_current[0, 0], int(point_other[0, 0]), int(step_length + 1))
    path_y = np.linspace(point_current[0, 1], int(point_other[0, 1]), int(step_length + 1))
...
def tree_plot(map_img, rrt_tree):
    ...
    while point_a_index != 0:
        ...
        cv2.line(map_img, vertex_tuple[int(point_a_index)][0: 2], vertex_tuple[int(point_b_index)][0: 2], (0, 0, 255),
                 4)
        point_a_index = point_b_index
    cv2.imshow("地图", map_img)  # 转为RGB显示
    cv2.waitKey()
